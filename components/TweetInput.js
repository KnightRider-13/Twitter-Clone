    ...
    <div className="flex space-x-3 p-3 border-b border-gray-700">
      ...
      {!loading && (<div className="w-full">
        ...
        <div className="relative mb-4">
          <div onClick={() => setImage(null)} className="absolute top-1 left-1 bg-[#272C26] rounded-full w-8 h-8 flex justify-center items-center cursor-pointer hover:bg-white hover:bg-opacity-10">
            <XIcon className="h-5" />
          </div>
        ...
        <div className="flex justify-between border-t border-gray-700 pt-4">
          ...
          <div className="flex space-x-0">
            <div className="iconsAnimation" onClick={() => filePickerRef.current.click()}>
          ...
          <button disabled={!text && !image} onClick={sendTweet} className="bg-[#1D9BF0] rounded-full px-4 py-2 disabled:opacity-50">Tweet</button>
        </div>
      </div>)}
    </div>
  );
